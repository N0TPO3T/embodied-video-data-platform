...
SOURCE = (
    ...
    / "superpowers"
    / "specs"
    / "2026-08-05-embodied-video-platform-greenfield-architecture-design.md"
)
OUTPUT_DIR = ROOT / "outputs" / "019fcabc-e414-7b02-addb-e06f9e8ba4e3"
OUTPUT = OUTPUT_DIR / "具身智能视频数据平台-整体需求与技术架构方案-v2.0.docx"
...
def configure_sections(doc: Document) -> None:
    for section in doc.sections:
        ...
        run = p.add_run("具身智能视频数据平台  |  整体需求与技术架构方案")
...
def add_cover(doc: Document) -> None:
    ...
    run = kicker.add_run("PRODUCT REQUIREMENTS & TECHNICAL ARCHITECTURE")
    ...
    run = subtitle.add_run("整体需求与技术架构方案")
    set_run_font(run, size=16, color=DARK_BLUE, bold=False)
    ...
    meta_rows = [
        ...
        ("推荐基线", "模块化单体 + PostgreSQL + 对象存储 + 消息队列 + 媒体/AI Worker"),
        ("使用场景", "需求评审、技术选型、系统设计、开发拆分与一期验收"),
    ]
    for label, value in meta_rows:
        ...
        set_run_font(label_run, size=10.5, color=BLUE, bold=True)
        value_run = p.add_run(value)
        set_run_font(value_run, size=10.5, color=INK)

    callout = doc.add_paragraph(style="Callout")
    callout.paragraph_format.space_before = Pt(20)
    ...
    add_inline(
        callout,
        "推荐定位：建设一条可运营、可追溯、可结算的视频数据生产流水线；"
        "业务服务保持简单，大文件直传对象存储，媒体与 AI 任务异步执行。",
        size=10.5,
        color=NAVY,
        bold=True,
    )
...
def add_navigation(doc: Document) -> None:
    ...
    p.paragraph_format.space_before = Pt(0)
    nav = [
        ("业务范围与需求", "第 0-5 章：目标、角色、一期/二期需求、状态和数据流"),
        ("架构与技术选型", "第 6-9 章：原则、三套候选、推荐逻辑架构和组件选型"),
        ("数据与容量", "第 10-12 章：数据模型、API/异步协议和 1000 用户容量"),
        ("质量与运行保障", "第 13-16 章：安全、部署、监控、恢复、测试和验收"),
        ("实施与决策", "第 17-19 章：工作量、开发顺序、待确认事项和最终推荐"),
    ]
...
def choose_widths(headers: list[str], rows: list[list[str]]) -> list[int]:
    cols = len(headers)
    if cols == 2:
        if headers[0] in {"角色", "组件", "API 组", "领域", "方案", "工作包", "层级", "进程", "场景", "数据"}:
            return [2200, 7160]
        return [1900, 7460]
    if cols == 3:
        if headers[0] == "ID":
...
def parse_markdown(doc: Document, markdown: str, bullet_num: int, decimal_num: int) -> None:
    ...
    while index < len(lines):
        ...
        if decimal_match:
            if decimal_match.group(1) == "1":
                current_decimal_num = add_numbering(doc, kind="decimal")
            p = doc.add_paragraph()
            apply_num(p, current_decimal_num)
            add_inline(p, decimal_match.group(2))
            index += 1
            continue
...
def build() -> None:
    ...
    doc.core_properties.title = "具身智能视频数据平台 - 整体需求与技术架构方案"
    doc.core_properties.subject = "需求评审、技术选型、系统设计、开发拆分与一期验收"
    doc.core_properties.author = "Codex"
    doc.core_properties.keywords = "具身智能, 视频数据, 微信小程序, PostgreSQL, 对象存储, 消息队列, AI"
