...
export function AdminDashboard() {
  ...
  return (
    <div className="page-stack">
      <div className="page-heading"><div><p className="page-kicker">全平台运营态势</p><h1>运营总览</h1><span>AI 质检使用 Qwen 模型路由，结果持久化保存到 PostgreSQL</span></div><span className="live-pill"><i />{mode === "live" ? "已连接后端数据" : mode === "loading" ? "正在读取数据" : "演示数据"}</span></div>
      ...
      <div className="dashboard-grid">
        <section className="content-card content-card-wide"><div className="card-heading"><div><h2>正式 AI 质检</h2><p>当前主流程使用 Qwen3.7 模型路由，可在“AI 系统提示词”中调整</p></div></div><div className="pipeline-list"><div><span>初检模型</span><strong>qwen3.7-plus</strong><em>当前生效</em></div><div><span>条件复核</span><strong>qwen3.7-flash</strong><em>按规则触发</em></div><div><span>并发上限</span><strong>3</strong><em>单 Worker 实例</em></div><div><span>结果存储</span><strong>PostgreSQL</strong><em>重启后保留</em></div></div></section>
