...
export function SettlementPage() {
  ...
  return (
    <div className="page-stack">
      ...
      <div className="dashboard-grid"><section className="content-card table-card"><div className="card-heading"><div><h2>结算批次</h2><p>每日锁定有效时长和应付金额</p></div></div><div className="table-scroll"><table className="data-table"><thead><tr><th>批次</th><th>日期</th><th>视频数</th><th>有效时长</th><th>金额</th><th>状态</th></tr></thead><tbody>{state.settlements.map((batch) => <tr key={batch.id}><td><strong>{batch.id}</strong></td><td>{batch.date}</td><td>{batch.submissionCount} 条</td><td>{batch.effectiveMinutes} 分钟</td><td><strong>¥{batch.amount.toFixed(2)}</strong></td><td><StatusBadge label={batch.status === "locked" ? "已锁定" : "处理中"} tone={batch.status === "locked" ? "success" : "info"}/></td></tr>)}</tbody></table></div></section><aside className="content-card"><div className="card-heading"><div><h2>质量系数</h2><p>最终评分对应计价倍率</p></div></div><div className="coefficient-list"><div><span>80 — 100 分</span><strong>1.00</strong><em>优质</em></div><div><span>70 — 79 分</span><strong>0.85</strong><em>合格</em></div><div><span>60 — 69 分</span><strong>0.70</strong><em>基础</em></div><div><span>低于 60 分</span><strong>0.00</strong><em>不计费</em></div></div></aside></div>
